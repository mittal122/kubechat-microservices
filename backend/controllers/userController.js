...
// @desc    Search users by name or email (partial match, case-insensitive)
...
const searchUsers = async (req, res) => {
  try {
    ...
    if (query.length < 3) {
      // Use Regex for partial matches on short queries
      const searchQuery = {
        $and: [
          { _id: { $ne: req.user._id } },
          {
      ...
      // Use highly optimized $text search for long queries
      const searchQuery = {
        $text: { $search: query },
        _id: { $ne: req.user._id },
      };
